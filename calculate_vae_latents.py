...
from longvgen.video_ipadapter import ResamplerV6
...
from longvgen.data.utils import ResolutionControl, resize_for_rectangle_crop
...
def prepare_attention_masks(
    ...
    attn_masks = arrange(attn_masks, "b f h w -> b (f h w)")
...
#    )
#    #state_dict = transformer.patch_embed.proj.cpu().state_dict()
#    #torch.save(state_dict, "patch_embed_proj.pt")
#    torch.save(transformer.patch_embed.proj.cpu(), "patch_embed_proj.pt")
#    raise IOError
#    patch_embed_proj = torch.load(
#        os.path.join(args.pretrained_model_name_or_path,"patch_embed_proj.pt")
#    )
...
#        resampler = ResamplerV6.from_pretrained(
...
#        resampler = ResamplerV6(**resampler_params)
    ...
    def load_model_hook(models, input_dir):
        ...
        while len(models) > 0:
            ...
            if isinstance(model, type(unwrap_model(transformer))):
                ...
                load_model = ResamplerV6.from_pretrained(
...
#        )
    ...
    num_trainable_parameters = 0 #sum(param.numel() for model in params_to_optimize for param in model["params"])
    ...
    def encode_video(video, valid_num_chunks, nf_per_chunk=49, resampler_image_rotary_emb=None, resampler_sampling_rotary_emb=None):
        ...
        latent_dist = torch.zeros((bsz, 13*num_chunks, 16, 60, 90)).to(accelerator.device, dtype=vae.dtype)

        for _b in range(bsz):
            print("encode video, bsz", _b)
            for i in range(min(num_chunks, valid_num_chunks[_b])):
                print("encode video, chunks", i)
...
#                latent_dist[[_b],i*4:(i+1)*4] = image_embeddings
                latent_dist[[_b],i*13:(i+1)*13] = latent

        print("latent_dist", latent_dist.min(), latent_dist.max(), latent_dist.mean(), latent_dist.std())
        #latent_dist = latent_dist * vip_params.scaling_factor
...
#                models_to_accumulate += [resampler,]
            ...
            print("pixel_values", pixel_values.shape, pixel_values.min(), pixel_values.max())
            valid_num_chunks = batch["valid_num_chunks"]
            video_names = batch["video_index"]
...
#                )
#            )
            model_input = encode_video(
                pixel_values,
                valid_num_chunks,
#                resampler_image_rotary_emb=resampler_image_rotary_emb,
#                resampler_sampling_rotary_emb=resampler_sampling_rotary_emb,
                resampler_image_rotary_emb=None,
                resampler_sampling_rotary_emb=None,
            )
#            valid_masks = prepare_loss_masks(
#                model_input, 
#                valid_num_chunks * resampler_params.num_temporal_queries,
#                model_input.device, 
#                model_input.dtype
#            ) 
            print("model_input.shape", model_input.shape)
            print("model_input value", model_input.min(), model_input.mean(), model_input.max()) 
            for _b in range(model_input.shape[0]):
                save_path = os.path.join(args.output_dir, video_names[_b]+f"_vae_c{valid_num_chunks[_b]}.pt")
                hf_save_path = os.path.join("4x8x12_1300_2", video_names[_b]+f"_c{valid_num_chunks[_b]}.pt")
                torch.save(
                    model_input[_b,:13*valid_num_chunks[_b]].cpu(), 
                    save_path
                ) 
#                flag = True
#                while flag:
...
#                os.system(f"rm -f {save_path}") 
            
            if accelerator.is_main_process:
#                filelist = os.listdir(args.output_dir)
#                for _path in filelist:
#                    if _path.endswith(".pt"):
#                        save_path = os.path.join(args.output_dir, _path)
#                        hf_save_path = os.path.join("4x8x12_1300", _path)
#                        flag = True
#                        while flag:
#                            try:
#                                print(f"Uploading {save_path}...")
...
#            num_samples += 1
#            sum_samples += torch.sum(model_input * valid_masks, dim=(0,1,3,4), keepdim=True) / torch.sum(valid_masks, dim=(0,1,3,4), keepdim=True)
#            mean = sum_samples / num_samples
#            print("current mean", mean)
#            #torch.save(mean.cpu(), os.path.join(args.output_dir, "mean.pt"))
#            mean = torch.load(vip_params.mean_path).to(dtype=mean.dtype, device=mean.device)
#            std_samples += torch.sum(((model_input - mean) ** 2) * valid_masks, dim=(0,1,3,4), keepdim=True) / (torch.sum(valid_masks, (0,1,3,4), keepdim=True)-1)
#            num_std_samples += 1
#            std = torch.sqrt(std_samples / num_std_samples)
#            print("current std", std)
#            torch.save(std.cpu(), os.path.join(args.output_dir, "std.pt"))
#            print("current std, min, max", std.min(), std.max())
