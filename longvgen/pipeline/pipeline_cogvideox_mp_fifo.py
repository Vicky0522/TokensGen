...
@dataclass
class FIFOCogVideoXPipelineOutput(BaseOutput):
    
    fifo_latents: torch.FloatTensor
    fifo_old_pred_original_sample: List[torch.FloatTensor]
    orig_latents: torch.FloatTensor
    nf_per_chunk: int
    vip_nf_per_chunk: int
    num_frames: int
    image_embeddings: torch.FloatTensor
    timesteps: torch.FloatTensor
    num_inference_steps: int
    do_classifier_free_guidance: bool
    use_separate_guidance: bool
    use_dynamic_cfg: bool
    prompt_embeds: torch.FloatTensor
    image_rotary_emb: Tuple[torch.FloatTensor]
    vip_image_rotary_grid: List[torch.FloatTensor]
    vip_condition_rotary_grid: List[torch.FloatTensor]
    cache_idx: List[int]
    attention_kwargs: Optional[Dict[str, Any]] = None
    guidance_scale: float = 6
    guidance_scale_img: float = 6 
    extra_step_kwargs: Optional[Dict[str, Any]] = None
    condition_frames: Optional[torch.FloatTensor] = None
    video_ipadapter_start_frame_idx: Optional[int] = 1000
    sampling_params: Dict[str, Any] = None
    output_type: str = "pil"
    return_dict: bool = True

class MPFIFOVideoIPAdapterCogVideoXPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    r"""
    ...
    def vae_encode_image(
        ...
        if do_classifier_free_guidance:
            ...
            if use_separate_guidance:
                image_embeddings = torch.cat([image_embeddings, uncond_image_embeddings, image_embeddings], dim=0)
            else:
                #image_embeddings = torch.cat([uncond_image_embeddings, image_embeddings], dim=0)
    ...
    def __call__(
        ...
        timesteps: Optional[List[int]] = None,
        use_separate_guidance: bool = False,
        guidance_scale: float = 6,
        guidance_scale_img: float = 6,
        ...
        video_ipadapter_start_frame_idx: Optional[int] = 1000,
    ) -> Union[FIFOCogVideoXPipelineOutput, Tuple]:
        """
            ...
            for i, t in enumerate(timesteps):
                ...
                if do_classifier_free_guidance:
                    if use_separate_guidance:
                        ...
                        noise_pred = noise_pred_txt_img + (self.guidance_scale-1) * (noise_pred_txt_img - noise_pred_uncond_txt) + (guidance_scale_img - 1) * (noise_pred_txt_img - noise_pred_uncond_img)
                    else:
        ...
        return FIFOCogVideoXPipelineOutput(
            ...
            use_separate_guidance=use_separate_guidance,
    ...
    def preprare_for_fifo(
        ...
        # 6. Prepare extra step kwargs. TODO: Logic should ideally just be moved out of the pipeline
        #self._extra_step_kwargs = self.prepare_extra_step_kwargs(generator, eta)
