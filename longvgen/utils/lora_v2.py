...
class LoraInjectedLinear(nn.Module):
    def __init__(
        ...
        self.tune_lora_down = nn.Linear(r, r, bias=False)
        self.tune_lora_up = nn.Linear(r, r, bias=False)
        self.dropout = nn.Dropout(dropout_p)
        self.scale = scale
        self.selector = nn.Identity()

        nn.init.normal_(self.tune_lora_down.weight, std=1 / r)
        nn.init.zeros_(self.tune_lora_up.weight)

    def forward(self, input):
        shared_lora_down_weight, shared_lora_up_weight = self.shared_lora_weight
        assert input.shape[-1] == shared_lora_down_weight.shape[-1]
        assert self.linear.weight.shape[0] == shared_lora_up_weight.shape[0]
        shared_lora_down_weight = rearrange(shared_lora_down_weight, "r in -> in r")
        lora_down_weight = rearrange(
            self.tune_lora_down(
                rearrange(shared_lora_down_weight, "r in -> in r")
            ),
            "in r -> r in"
        )
        lora_up_weight = self.tune_lora_down(
                shared_lora_up_weight
        )
        return (
            self.linear(input)
            + self.dropout(F.linear(self.selector(F.linear(input, lora_down_weight)),lora_up_weight))
            * self.scale
        )
    ...
    def set_selector_from_diag(self, diag: torch.Tensor):
        ...
        self.selector.weight.data = self.selector.weight.data.to(
            self.tune_lora_up.weight.device
        ).to(self.tune_lora_up.weight.dtype)
    ...
    for ancestor in ancestors:
        for fullname, module in ancestor.named_modules():
            if any([isinstance(module, _class) for _class in search_class]):
                ...
                #    continue_flag = False
                #if 'BasicTransformerBlock' in ancestor_class and ('attn2' in fullname or 'attn1' in fullname or 'ff' in fullname) and ('_ip' not in fullname):
                #    continue_flag = False
                if 'SpatialTransformer' in ancestor_class and ('attn1' in fullname or 'ff' in fullname) and ('_ip' not in fullname): 
                    continue_flag = False
                if 'TemporalTransformer' in ancestor_class and ('attn1' in fullname or 'attn2' in fullname or 'ff' in fullname) and ('_ip' not in fullname):
                    continue_flag = False
                #if 'ResBlock' in ancestor_class:
                #    continue_flag = False
                if continue_flag:
                    continue
    ...
    if True:
        for target_replace_module_i in target_replace_module:
            for _module, name, _child_module in _find_modules(
                ...
                if loras != None:
                    _module._modules[name].lora_up.weight.data.copy_(loras.pop(0))
                    _module._modules[name].lora_down.weight.data.copy_(loras.pop(0))
