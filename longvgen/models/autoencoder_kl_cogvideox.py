...
class AutoencoderKLCogVideoX(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def tiled_decode(self, z: torch.Tensor, return_dict: bool = True) -> Union[DecoderOutput, torch.Tensor]:
        ...
        for i in range(0, height, overlap_height):
            ...
            for j in range(0, width, overlap_width):
                nf_per_chunk = 13
                num_chunks = num_frames // nf_per_chunk
                num_batches = nf_per_chunk // frame_batch_size
                time = []
                for k1 in range(num_chunks):
                    for k2 in range(num_batches):
                        remaining_frames = nf_per_chunk % frame_batch_size
                        start_frame = k1 * nf_per_chunk + frame_batch_size * k2 + (0 if k2 == 0 else remaining_frames)
                        end_frame = k1 * nf_per_chunk + frame_batch_size * (k2 + 1) + remaining_frames
                        tile = z[
                            :,
                            :,
                            start_frame:end_frame,
