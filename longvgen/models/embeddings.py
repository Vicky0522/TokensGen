...
def get_3d_sincos_pos_embed_v2(
    embed_dim: int,
    grid_h = None,
    grid_w = None,
    grid_t = None,
    ...
    if embed_dim % 4 != 0:
        raise ValueError("`embed_dim` must be divisible by 4")
    spatial_size = (grid_w.shape[0], grid_h.shape[0])
...
class CogVideoXPatchEmbed(nn.Module):
    ...
    def set_vip_layers(self, **kwargs):
        self.use_vip = True
        self.func_type = kwargs.get("func_type", "1")

        vip_embed_dim = kwargs["resampler_params"]["output_dim"]
        self.vip_proj = nn.Linear(vip_embed_dim, self.embed_dim)
        self.vip_num_height_queries = kwargs["resampler_params"]["num_height_queries"]
        self.vip_num_width_queries = kwargs["resampler_params"]["num_width_queries"]
        self.vip_num_temporal_queries = kwargs["resampler_params"]["num_temporal_queries"]
#        self.video_ipadapter_start_frame_idx = kwargs["video_ipadapter_start_frame_idx"]

#        if self.use_positional_embeddings or self.use_learned_positional_embeddings:
#            persistent = self.use_learned_positional_embeddings
#            pos_embedding = self._get_positional_embeddings(
#                kwargs["resampler_params"]["num_height_queries"],
#                kwargs["resampler_params"]["num_width_queries"],
#                kwargs["resampler_params"]["num_temporal_queries"]+1,
#                self.sample_height//self.patch_size,
#                self.sample_width//self.patch_size,
#                (self.sample_frames - 1) // self.temporal_compression_ratio + 1,
#                1, 1
#            )
#            pos_embedding = self._get_joint_positional_embeddings(self._pos_embedding, pos_embedding)
#            self.register_buffer("pos_embedding", pos_embedding, persistent=persistent)
    ...
    def _get_vip_positional_embeddings(
        self, sample_height: int, sample_width: int, 
        grid_t: np.ndarray
    ) -> torch.Tensor:
        
        pos_embedding = []

        for _grid_t in grid_t:
            pos_embedding.append(get_3d_sincos_pos_embed_v2(
                self.embed_dim,
                grid_h = np.linspace(
                    0,
                    sample_height // self.patch_size, 
                    self.vip_num_height_queries,
                    endpoint=False,
                    dtype=np.float32,
                ) / self.spatial_interpolation_scale,
                grid_w = np.linspace(
                    0,
                    sample_width // self.patch_size,
                    self.vip_num_width_queries,
                    endpoint=False,
                    dtype=np.float32,
                ) / self.spatial_interpolation_scale,
                grid_t = _grid_t
        ...
        pos_embedding = np.stack(pos_embedding)
        pos_embedding = torch.from_numpy(pos_embedding).flatten(1, 2)
    ...
    def forward(self, 
        ...
        if self.use_vip:
            _, vip_num_frames, _, vip_height, vip_width = vip_embeds.shape
            vip_embeds = rearrange(vip_embeds, "b f c h w -> b (f h w) c")
            vip_embeds = self.vip_proj(vip_embeds)
        ...
        concat_embeds = [text_embeds, image_embeds]
        if self.use_vip:
            concat_embeds = concat_embeds + [vip_embeds,]
        embeds = torch.cat(concat_embeds, dim=1).contiguous()  # [batch, seq_length + num_frames x height x width, channels]
        ...
        if self.use_positional_embeddings or self.use_learned_positional_embeddings:
            ...
            if (
                ...
                if self.use_vip:
                    vip_pos_embedding = self._get_vip_positional_embeddings(
                        self.sample_height,
                        self.sample_width,
                        grid_t
                    ).to(device=pos_embedding.device, dtype=pos_embedding.dtype)
                    pos_embedding = repeat(pos_embedding, "1 n d -> b n d", b=vip_pos_embedding.shape[0])
                    pos_embedding = torch.cat([pos_embedding, vip_pos_embedding], dim=1)
                    pos_embedding = pos_embedding.to(embeds.device, dtype=embeds.dtype)
...
def get_3d_rotary_pos_embed(
    embed_dim, 
    crops_coords, #[[f0, h0, w0], [f1, h1, w1]] 
    grid_size, # [F, H, W]
    ...
    temporal_size, grid_size_h, grid_size_w = grid_size
    grid_t = np.linspace(start[0], stop[0], temporal_size, endpoint=False, dtype=np.float32)
...
def get_3d_rotary_pos_embed_v2(
    embed_dim, 
    grid_t, grid_h, grid_w,
    dim_t: int = None, dim_h: int = None, dim_w: int = None,
    theta: int = 10000, use_real: bool = True
) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
    """
    RoPE for video tokens with 3D structure.
    ...
    if use_real is not True:
        raise ValueError(" `use_real = False` is not currently supported for get_3d_rotary_pos_embed")
    temporal_size = len(grid_t)
    grid_size_h = len(grid_h)
    grid_size_w = len(grid_w)

    # Compute dimensions for each axis
    dim_t = embed_dim // 4 if dim_t is None else dim_t
    dim_h = embed_dim // 8 * 3 if dim_h is None else dim_h
    dim_w = embed_dim // 8 * 3 if dim_w is None else dim_w
...
def get_1d_rotary_pos_embed(
    dim: int,
    pos: Union[np.ndarray, torch.Tensor, int],
    theta: float = 10000.0,
    use_real=False,
    linear_factor=1.0,
    ntk_factor=1.0,
...
def apply_rotary_emb(
    ...
    if use_real:
        ...
        if cos.ndim == 2 and sin.ndim == 2:
            cos = cos[None, None]
            sin = sin[None, None]
        elif cos.ndim == 3 and sin.ndim == 3:
            cos = cos.unsqueeze(1)
            sin = sin.unsqueeze(1)
        else:
            pass
