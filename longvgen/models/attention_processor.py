...
class Attention(nn.Module):
    ...
    def forward(
        ...
        **cross_attention_kwargs,
        ...
        quiet_attn_parameters = {"ip_adapter_masks", "vip_image_rotary_emb", "vip_condition_rotary_emb"}
        unused_kwargs = [
...
class VideoIPAdapterCogVideoXAttnProcessor2_0(nn.Module):
    r"""
    Processor for implementing scaled dot-product attention for the CogVideoX model. It applies a rotary embedding on
    query and key vectors, but does not include spatial normalization.
    """

    def __init__(self, heads, cross_attention_dim=None, dim_head=None, eps=1e-6, scale=1.0, qk_norm=None, bias=False, num_tokens=None, **kwargs):
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError("CogVideoXAttnProcessor requires PyTorch 2.0, to use it, please upgrade PyTorch to 2.0.")
        super().__init__()
        ...
        self.vip_to_q = nn.Linear(cross_attention_dim, hidden_size, bias=bias)
        self.vip_to_k = nn.Linear(cross_attention_dim, hidden_size, bias=bias)
        self.vip_to_v = nn.Linear(cross_attention_dim, hidden_size, bias=bias)

        if qk_norm == "layer_norm": 
            self.vip_norm_q = nn.LayerNorm(dim_head, eps=eps)
            self.vip_norm_k = nn.LayerNorm(dim_head, eps=eps)
        else:
            self.vip_norm_q = None
            self.vip_norm_k = None
    ...
    def __call__(
        ...
        attention_mask: Optional[torch.Tensor] = None,
        image_rotary_emb: Optional[torch.Tensor] = None,
        vip_image_rotary_emb: Optional[torch.Tensor] = None,
        vip_condition_rotary_emb: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        encoder_hidden_states, vip_encoder_hidden_states = encoder_hidden_states.split(
            [encoder_hidden_states.shape[1] - self.num_tokens, self.num_tokens],
            dim=1
        )
        text_seq_length = encoder_hidden_states.size(1)
        vip_seq_length = vip_encoder_hidden_states.size(1)
        ...
        query_text_video = self.vip_to_q(hidden_states)
        query_vip = self.vip_to_q(vip_encoder_hidden_states)
        key_text_video = self.vip_to_k(hidden_states)
        key_vip = self.vip_to_k(vip_encoder_hidden_states)
        value_text_video = self.vip_to_v(hidden_states)
        value_vip = self.vip_to_v(vip_encoder_hidden_states)
        ...
        (
            query, key, value, 
            query_text_video,query_vip,key_text_video,key_vip,value_text_video,value_vip
        ) = map(
            lambda t: t.view(batch_size, -1, attn.heads, head_dim)
            .transpose(1, 2),
            (query,key,value,query_text_video,query_vip,key_text_video,key_vip,value_text_video,value_vip))

        if attn.norm_q is not None:
            query = attn.norm_q(query)
        if attn.norm_k is not None:
            key = attn.norm_k(key)
        if self.vip_norm_q is not None:
            query_text_video = self.vip_norm_q(query_text_video)
            query_vip = self.vip_norm_q(query_vip)
        if self.vip_norm_k is not None:
            key_text_video = self.vip_norm_k(key_text_video)
            key_vip = self.vip_norm_k(key_vip)

        # Apply RoPE if needed
        if image_rotary_emb is not None:
            from .embeddings import apply_rotary_emb

            #tmp1, tmp2 = image_rotary_emb
            #print(query[:, :, text_seq_length:].shape)
            #print(tmp1.shape, tmp2.shape)
            #print(text_seq_length)
            query[:, :, text_seq_length:] = apply_rotary_emb(query[:, :, text_seq_length:], image_rotary_emb)
            query_text_video[:,:,text_seq_length:] = apply_rotary_emb(query_text_video[:,:,text_seq_length:], vip_image_rotary_emb)
            query_vip = apply_rotary_emb(query_vip, vip_condition_rotary_emb)
            #if not attn.is_cross_attention:
            key[:, :, text_seq_length:] = apply_rotary_emb(key[:, :, text_seq_length:], image_rotary_emb)
            key_text_video[:,:,text_seq_length:] = apply_rotary_emb(key_text_video[:,:,text_seq_length:], vip_image_rotary_emb)
            key_vip = apply_rotary_emb(key_vip, vip_condition_rotary_emb)

#        (
#            query, key, value, 
#            query_text_video,query_vip,key_text_video,key_vip,value_text_video,value_vip
#        ) = map(
#            lambda t: t.reshape(batch_size * attn.heads, t.shape[2], head_dim)
#            .contiguous(),
#            (query,key,value,query_text_video,query_vip,key_text_video,key_vip,value_text_video,value_vip))
...
#            ## video ip-adapter
#            text_video_hidden_states = list() 
#            for i_batch in range(n_batches):
#                batch = slice(i_batch * max_bs, (i_batch + 1) * max_bs) 
#                text_video_hidden_states.append(
#                    xformers.ops.memory_efficient_attention(
#                        query_text_video[batch],
#                        key_vip[batch],
#                        value_vip[batch],
#                        attn_bias=None,
#                        op=None
#                    )
#                )
#                text_video_hidden_states = torch.cat(text_video_hidden_states, 0)
#            vip_hidden_states = list() 
#            key_text_video_vip = torch.cat([key_text_video, key_vip], dim=1)
#            value_text_video_vip = torch.cat([value_text_video, value_vip], dim=1)
#            for i_batch in range(n_batches):
#                batch = slice(i_batch * max_bs, (i_batch + 1) * max_bs) 
#                vip_hidden_states.append(
#                    xformers.ops.memory_efficient_attention(
#                        query_vip[batch],
#                        key_text_video_vip[batch],
#                        value_text_video_vip[batch],
#                        attn_bias=None,
#                        op=None
#                    )
#                )
#                vip_hidden_states = torch.cat(vip_hidden_states, 0)

            text_video_hidden_states = F.scaled_dot_product_attention(
                query_text_video, key_vip, value_vip, attn_mask=attention_mask, dropout_p=0.0,is_causal=False 
            )
            vip_hidden_states = F.scaled_dot_product_attention(
                query_vip, 
                torch.cat([key_text_video, key_vip], dim=2),
                torch.cat([value_text_video, value_vip], dim=2),
                attn_mask=attention_mask, dropout_p=0.0, is_causal=False
            )
        scale = torch.Tensor(self.scale).to(
            device = text_video_hidden_states.device,
            dtype = text_video_hidden_states.dtype
        )
        if scale.ndim > 0 and scale.shape[0] != text_video_hidden_states.shape[0]:
            scale = scale[0]
        while scale.ndim < text_video_hidden_states.ndim:
            scale = scale.unsqueeze(-1)
        hidden_states = hidden_states + scale * text_video_hidden_states
        hidden_states = torch.cat([hidden_states, vip_hidden_states], dim=2)

#        hidden_states = (
#            hidden_states.unsqueeze(0)
#            .reshape(batch_size, attn.heads, hidden_states.shape[1], head_dim)
#            .permute(0, 2, 1, 3)
...
#        )
        ...
        encoder_hidden_states, hidden_states, vip_hidden_states = hidden_states.split(
            [text_seq_length, hidden_states.size(1) - text_seq_length - vip_seq_length, vip_seq_length], 
            dim=1
        )
        encoder_hidden_states = torch.cat([encoder_hidden_states, vip_hidden_states], dim=1)
        return hidden_states, encoder_hidden_states

class VideoIPAdapterV2CogVideoXAttnProcessor2_0(nn.Module):
    r"""
    Processor for implementing scaled dot-product attention for the CogVideoX model. It applies a rotary embedding on
    query and key vectors, but does not include spatial normalization.
    """

    def __init__(self, heads, cross_attention_dim=None, dim_head=None, eps=1e-6, scale=1.0, qk_norm=None, bias=False, num_tokens=None):
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError("CogVideoXAttnProcessor requires PyTorch 2.0, to use it, please upgrade PyTorch to 2.0.")
        super().__init__()
    ...
    def __call__(
        ...
        query_text_video = self.vip_to_q(hidden_states[:,text_seq_length:])
        query_vip = self.vip_to_q(vip_encoder_hidden_states)
        key_text_video = self.vip_to_k(hidden_states[:,text_seq_length:])
        key_vip = self.vip_to_k(vip_encoder_hidden_states)
        value_text_video = self.vip_to_v(hidden_states[:,text_seq_length:])
        value_vip = self.vip_to_v(vip_encoder_hidden_states)
        ...
        (
            ...
            lambda t: t.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2),
            (query,key,value,query_text_video,query_vip,key_text_video,key_vip,value_text_video,value_vip))
        ...
        if image_rotary_emb is not None:
            ...
            query[:, :, text_seq_length:] = apply_rotary_emb(query[:, :, text_seq_length:], image_rotary_emb)
            query_text_video = apply_rotary_emb(query_text_video, vip_image_rotary_emb)
            query_vip = apply_rotary_emb(query_vip, vip_condition_rotary_emb)
            #if not attn.is_cross_attention:
            key[:, :, text_seq_length:] = apply_rotary_emb(key[:, :, text_seq_length:], image_rotary_emb)
            key_text_video = apply_rotary_emb(key_text_video, vip_image_rotary_emb)
            key_vip = apply_rotary_emb(key_vip, vip_condition_rotary_emb)
        ...
        hidden_states[:,:,text_seq_length:] = hidden_states[:,:,text_seq_length:] + self.scale * text_video_hidden_states
        hidden_states = torch.cat([hidden_states, vip_hidden_states], dim=2)
...
class VideoIPAdapterV3CogVideoXAttnProcessor2_0(nn.Module):
    r"""
    Processor for implementing scaled dot-product attention for the CogVideoX model. It applies a rotary embedding on
    query and key vectors, but does not include spatial normalization.
    """
    ...
    def __call__(
        ...
        (
            query, key, value, 
            query_text_video, key_vip, value_vip
            #query_text_video,query_vip,key_text_video,key_vip,value_text_video,value_vip
        ) = map(
            lambda t: t.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2),
            #(query,key,value,query_text_video,query_vip,key_text_video,key_vip,value_text_video,value_vip)
            (query,key,value,query_text_video, key_vip, value_vip)
        )
...
#        )
        ...
        hidden_states = hidden_states.transpose(1, 2).reshape(batch_size, -1, attn.heads * head_dim)
#        hidden_states = torch.cat([hidden_states, vip_encoder_hidden_states], dim=1)
        ...
        encoder_hidden_states = torch.cat([encoder_hidden_states, vip_encoder_hidden_states], dim=1)
        return hidden_states, encoder_hidden_states

class VideoIPAdapterV4CogVideoXAttnProcessor2_0(nn.Module):
    r"""
    Processor for implementing scaled dot-product attention for the CogVideoX model. It applies a rotary embedding on
    query and key vectors, but does not include spatial normalization.
    """

    def __init__(self, heads, cross_attention_dim=None, dim_head=None, eps=1e-6, scale=1.0, scale_ar=1.0, qk_norm=None, bias=False, num_tokens=None, ar_num_tokens=None):
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError("CogVideoXAttnProcessor requires PyTorch 2.0, to use it, please upgrade PyTorch to 2.0.")
        super().__init__()
        ...
        self.cross_attention_dim = cross_attention_dim
        self.scale = scale
        self.scale_ar = scale_ar
        self.num_tokens = num_tokens
        self.ar_num_tokens = ar_num_tokens
    ...
    def __call__(
        ...
        text_seq_length = encoder_hidden_states.size(1)
        vip_seq_length = vip_encoder_hidden_states.size(1)
        ar_seq_length = self.ar_num_tokens
        ar_hidden_states, hidden_states = hidden_states.split(
            [ar_seq_length, hidden_states.shape[1]-ar_seq_length],
            dim=1
        )
        ...
        query_text_video = self.vip_to_q(hidden_states)
        query_vip = self.vip_to_q(vip_encoder_hidden_states)
        query_ar = self.vip_to_q(ar_hidden_states)
        key_text_video = self.vip_to_k(hidden_states)
        key_vip = self.vip_to_k(vip_encoder_hidden_states)
        key_ar = self.vip_to_k(ar_hidden_states)
        value_text_video = self.vip_to_v(hidden_states)
        value_vip = self.vip_to_v(vip_encoder_hidden_states)
        value_ar = self.vip_to_v(ar_hidden_states)
        ...
        (
            query, key, value, 
            query_text_video,query_vip,key_text_video,key_vip,value_text_video,value_vip,
            query_ar,key_ar,value_ar
        ) = map(
            lambda t: t.view(batch_size, -1, attn.heads, head_dim)
            .transpose(1, 2),
            (query,key,value,query_text_video,query_vip,key_text_video,key_vip,value_text_video,value_vip,query_ar,key_ar,value_ar))
        ...
        if self.vip_norm_q is not None:
            query_text_video = self.vip_norm_q(query_text_video)
            query_vip = self.vip_norm_q(query_vip)
            query_ar = self.vip_norm_q(query_ar)
        if self.vip_norm_k is not None:
            key_text_video = self.vip_norm_k(key_text_video)
            key_vip = self.vip_norm_k(key_vip)
            key_ar = self.vip_norm_k(key_ar)
        ...
        if image_rotary_emb is not None:
            ...
            query[:, :, text_seq_length:] = apply_rotary_emb(query[:, :, text_seq_length:], image_rotary_emb)
            combined_query_ar_video = torch.cat(
                 [query_ar,
                 query_text_video[:,:,text_seq_length:]],
                dim=2
            )
            combined_query_ar_video = apply_rotary_emb(combined_query_ar_video, vip_image_rotary_emb)
            query_text_video[:,:,text_seq_length:] = combined_query_ar_video[:, :, ar_seq_length:]
            query_ar = combined_query_ar_video[:, :, :ar_seq_length]
            query_vip = apply_rotary_emb(query_vip, vip_condition_rotary_emb)
            #if not attn.is_cross_attention:
            key[:, :, text_seq_length:] = apply_rotary_emb(key[:, :, text_seq_length:], image_rotary_emb)
            combined_key_ar_video = torch.cat(
                [key_ar,
                 key_text_video[:,:,text_seq_length:]],
                dim=2
            )
            combined_key_ar_video = apply_rotary_emb(combined_key_ar_video, vip_image_rotary_emb)
            key_text_video[:,:,text_seq_length:] = combined_key_ar_video[:, :, ar_seq_length:]
            key_ar = combined_key_ar_video[:, :, :ar_seq_length]
            key_vip = apply_rotary_emb(key_vip, vip_condition_rotary_emb)
...
#                vip_hidden_states = torch.cat(vip_hidden_states, 0)
            ...
            text_video_hidden_states_1 = F.scaled_dot_product_attention(
                query_text_video, 
                key_vip,
                value_vip,
                attn_mask=attention_mask, dropout_p=0.0,is_causal=False 
            )
            text_video_hidden_states_2 = F.scaled_dot_product_attention(
                query_text_video, 
                key_ar,
                value_ar,
                attn_mask=attention_mask, dropout_p=0.0,is_causal=False 
            )
            ar_vip_hidden_states = F.scaled_dot_product_attention(
                torch.cat([query_ar,query_vip],dim=2), 
                torch.cat([key_text_video, key_ar, key_vip], dim=2),
                torch.cat([value_text_video, value_ar, value_vip], dim=2),
                attn_mask=attention_mask, dropout_p=0.0, is_causal=False
            )
            ar_hidden_states, vip_hidden_states = ar_vip_hidden_states.split(
                [ar_seq_length, vip_seq_length], dim=2)

        scale = torch.Tensor(self.scale).to(
            device = text_video_hidden_states_1.device,
            dtype = text_video_hidden_states_1.dtype
        )
        if scale.ndim > 0 and scale.shape[0] != text_video_hidden_states_1.shape[0]:
            scale = scale[0]
        while scale.ndim < text_video_hidden_states_1.ndim:
            scale = scale.unsqueeze(-1)
        hidden_states = hidden_states + scale * text_video_hidden_states_1
        hidden_states = hidden_states + self.scale_ar * text_video_hidden_states_2
