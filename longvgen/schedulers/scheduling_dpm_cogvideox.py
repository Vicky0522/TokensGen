...
class CogVideoXDPMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(
        ...
        if beta_schedule == "vip_1":
            print("use vip_1")
            a1_indx = int(num_train_timesteps*0.5)
            new_a1_index = int(num_train_timesteps*0.3)
            a2_indx = int(num_train_timesteps*0.8)
            new_a2_index = int(num_train_timesteps*0.5)
            #a3_indx = int(num_train_timesteps*0.7)
            #new_a3_index = int(num_train_timesteps*0.9)
            def _shift(a, b, na, nb):
                return (self.alphas_cumprod[na:nb] - self.alphas_cumprod[na]) / (self.alphas_cumprod[nb-1] - self.alphas_cumprod[na]) * (self.alphas_cumprod[b-1] - self.alphas_cumprod[a]) + self.alphas_cumprod[a]
            
            alphas_cumprod_1 = _shift(0, a1_indx, 0, new_a1_index)
            alphas_cumprod_2 = _shift(a1_indx, a2_indx, new_a1_index, new_a2_index)
            #alphas_cumprod_3 = _shift(a2_indx, a3_indx, new_a2_index, new_a3_index)
            alphas_cumprod_3 = _shift(a2_indx, num_train_timesteps, new_a2_index, num_train_timesteps)
    ...
    def add_noise_to_xt(
        self,
        xt_previous: torch.Tensor,
        noise: torch.Tensor,
        timesteps: torch.IntTensor,
     ) -> torch.Tensor:
        self.betas = self.betas.to(device=xt_previous.device)
        betas = self.betas.to(device=xt_previous.device)
        timesptes = timesteps.to(device=xt_previous.device)

        sqrt_one_minus_beta = (1 - betas[timesptes]) ** 0.5
        sqrt_beta = betas[timesptes] ** 0.5

        while len(sqrt_beta.shape) < len(xt_previous.shape):
            sqrt_beta = sqrt_beta.unsqueeze(-1)

        while len(sqrt_one_minus_beta.shape) < len(xt_previous.shape):
            sqrt_one_minus_beta = sqrt_one_minus_beta.unsqueeze(-1)

        xt = sqrt_one_minus_beta * xt_previous + sqrt_beta * noise
